...
// ============================================================================
// 1. ВАЛИДАЦИЯ ФОРМ (HTMX)
// ============================================================================

document.addEventListener('htmx:configRequest', function (evt) {
    const form = evt.detail.elt.closest('form');

    // Проверяем, есть ли у формы класс validate-required (добавлен в HTML)
    if (!form || !form.classList.contains('validate-required')) {
        return;
    }
    ...
    if (hasErrors) {
        evt.preventDefault(); // Отменяем отправку запроса
...
function taskManager(containerId) {
    return {
        ...
        handleDrop(event) {
            ...
            // Получаем CSRF-токен напрямую из DOM
