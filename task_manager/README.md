...
![demo](static/img/as_it_shows_now.png)
...
```bash
git clone https://github.com/Shrekulka/task_manager
cp .env.example .env
```

Для локального запуска значения по умолчанию подходят.

### 2. Новый раздел "🔧 Django Admin"

**Где добавить:** После раздела `## 📦 Быстрый запуск через Docker`.

**Что добавить:** Этот блок.

```markdown
## 🔧 Django Admin

После запуска приложения управлять данными можно через админ-панель Django:
...
```
Приложение будет доступно по адресу http://localhost:8000. Изменения в коде применяются автоматически (благодаря volume mount).

### 4. Запуск в режиме продакшена (production)
...
Примечание: Миграции и создание суперпользователя выполняются автоматически при первом запуске (благодаря entrypoint.sh).
