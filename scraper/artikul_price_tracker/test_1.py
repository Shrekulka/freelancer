...
def get_product_data(driver, article):
    ...
    # Принудительная прокрутка страницы до нужного элемента (если требуется)
    driver.execute_script("window.scrollTo(0, document.body.scrollHeight);")

    # Ожидаем появления блока с информацией о продавце
    try:
        WebDriverWait(driver, 20).until(
            EC.presence_of_element_located((By.CLASS_NAME, "seller-info__name"))
        )
    except Exception as e:
        print(f"Ошибка ожидания элемента продавца для артикула {article}: {e}")
    ...
    # Парсим название продавца
    seller_name_tag = soup.find("div", {"class": "seller-info__name"})
    seller_name = seller_name_tag.text.strip() if seller_name_tag else "Продавец не найден"
